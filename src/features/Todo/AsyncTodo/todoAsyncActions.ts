...
export const fetchAllTodos = createAsyncThunk<
	Todo[], undefined, { state: { asyncTodos: TodoSlice } }
>(
	'todos/fetchTodos',
	async () => {
...
export const removeTodo = createAsyncThunk<Todo['id'], Todo['id'], { rejectValue: string }>(
	...
	async (id: Todo['id'], { rejectWithValue }) => {
		...
		if (!response.ok) {
			return rejectWithValue('I can\'t delete todo with ID:' + id)
		}
		return id
	}
)

export const toggleTodo = createAsyncThunk<Todo, Todo['id'], { state: { asyncTodos: TodoSlice }, rejectValue: string }>(
	'todo/toggleTodo',
	async (id: Todo['id'], { getState, rejectWithValue }) => {
		...
		if (todo) {
			...
			if (!response.ok) {
				return rejectWithValue('I can\'t update todo with ID:' + id)
			}
